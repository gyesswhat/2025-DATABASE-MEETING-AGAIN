...
public class LeaderController {
	...
	private void createReservationTableIfNotExists() {
		if (!tableExists("db2025_reservation")) {
			String createTableSQL = "CREATE TABLE IF NOT EXISTS db2025_reservation ("
					...
					+ "people_count INT NOT NULL," + "reservation_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,"
	...
	public String getTeamMemberTimeInfo(int teamId) {
		// TODO Auto-generated method stub
		String sql = "SELECT * FROM db2025_team_user_time_view WHERE team_id=?";
		...
		try (Connection conn = DBUtil.getConnection(); 
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					sb.append("ID: ").append(rs.getInt("user_id")).append(" | Name: ").append(rs.getString("userName")).append(" | ").append(rs.getString("startTime")).append(" ~ ").append(rs.getString("endTime")).append(" | Priority: ").append(rs.getInt("priority")).append("\n");
